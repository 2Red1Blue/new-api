	...
	return func(c *gin.Context) {
		...
		if pin, found, overridden := constraints.ResolvedPin(); found {
			...
			if shouldSelectChannel {
				...
				if preferredChannelID, found := service.GetPreferredChannelByAffinity(c, modelRequest.Model, usingGroup); found {
					...
					if err == nil && preferred != nil {
						if preferred.Status != common.ChannelStatusEnabled {
							if service.ShouldSkipRetryAfterChannelAffinityFailure(c) && !service.ShouldBreakChannelAffinityOnUnavailable(c) {
								abortWithOpenAiMessage(c, http.StatusForbidden, i18n.T(c, i18n.MsgDistributorAffinityChannelDisabled))
								return
							}
						} else if affinitySatisfied, _ := model.ChannelSatisfiesFilters(preferred, modelRequest.Model, constraints.Filters); affinitySatisfied {
				...
				// 记录本次实际使用的真实分组，供亲和性绑定写入正确的 RealGroup
		...
		if channel != nil && c.Writer != nil && c.Writer.Status() < http.StatusBadRequest {
			service.RecordChannelAffinityFromContext(c)
		}
	}
}
