...
func Relay(c *gin.Context, relayFormat types.RelayFormat) {
	...
	for ; retryParam.GetRetry() <= common.RetryTimes; retryParam.IncreaseRetry() {
		...
		if willRetry && decision.Action != "retry" {
			decision = service.PolicyDecision{Action: "retry", Reason: "channel_affinity_failover", Source: "channel_affinity"}
		} else if !willRetry && decision.Action == "retry" {
			decision = service.PolicyDecision{Action: "stop", Reason: "local_retry_exclusion", Source: "channel_affinity"}
		}
		service.RecordPolicyFailure(c, channel.Id, newAPIError, decision)

		chErr := processChannelError(c, *types.NewChannelError(channel.Id, channel.Type, channel.Name, channel.ChannelInfo.IsMultiKey, common.GetContextKeyString(c, constant.ContextKeyChannelKey), channel.GetAutoBan()), newAPIError, relayInfo)
...
func shouldRetry(c *gin.Context, openaiErr *types.NewAPIError, retryTimes int, forceRetry ...bool) bool {
	...
	force := len(forceRetry) > 0 && forceRetry[0]
	code := openaiErr.StatusCode
	isRateLimit := code == http.StatusTooManyRequests
	isUnavailable := force || c.GetBool(channelRateLimitedContextKey) || code >= 500 || code < 100 || code > 599
	if isRateLimit {
...
func processChannelError(c *gin.Context, channelError types.ChannelError, err *types.NewAPIError, relayInfo *relaycommon.RelayInfo) channelErrorResult {
