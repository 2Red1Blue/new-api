...
func registerChannelRoutes(apiRouter *gin.RouterGroup) {
	...
	channelRoute.GET("/upstream_password/feature",
		middleware.RootAuth(),
		controller.GetUpstreamPasswordFeature,
	)
	channelRoute.POST("/upstream_group_ratios",
		middleware.RequirePermission(authz.ChannelRead),
		controller.PreviewChannelUpstreamGroupRatios,
	)
	channelRoute.GET("/:id/upstream_group_ratios",
		middleware.RequirePermission(authz.ChannelRead),
		controller.FetchChannelUpstreamGroupRatios,
	)
	...
	channelRoute.POST("/:id/upstream_password",
		middleware.RootAuth(),
		middleware.CriticalRateLimit(),
		middleware.DisableCache(),
		middleware.SecureVerificationRequired(),
		controller.GetChannelUpstreamPassword,
	)
	authSessionRoute := channelRoute.Group("/:id/upstream_auth")
	authSessionRoute.Use(
		middleware.RootAuth(),
		middleware.CriticalRateLimit(),
		middleware.DisableCache(),
		middleware.SecureVerificationRequired(),
