...
export function ChannelMutateDrawer({
  ...
  const handleRevealUpstreamPassword = useCallback(async () => {
    ...
    try {
      ...
      const res = await getChannelUpstreamPassword(channelId, proof.proof_token)
      if (!res.success) {
        throw createServerError(res, t('Failed to fetch upstream password'))
      }
      setUpstreamPassword(res.data?.password ?? '')
      toast.success(t('Upstream password unlocked'))
    } catch (error) {
      handleServerError(error, t('Failed to fetch upstream password'))
    } finally {
      setIsUpstreamPasswordLoading(false)
    }
  }, [channelId, t, verification])
  ...
  const upstreamInformationFields = (
    <div className='border-border/60 bg-muted/20 space-y-4 rounded-lg border p-4'>
      <div className='flex items-start justify-between gap-3'>
        <div>
          <p className='text-sm font-medium'>{t('Upstream Information')}</p>
          <p className='text-muted-foreground text-xs'>
            {t(
        ...
        <Button
          ...
          disabled={isUpstreamRatiosLoading || sensitiveLocked}
        >
      ...
      <FormField
        ...
        render={({ field }) => (
          <FormItem>
            ...
            {isEditing && canRevealUpstreamPassword && (
              <div className='flex gap-2'>
                <Button
                  ...
                  disabled={isUpstreamPasswordLoading || verification.isActive}
                >
                  {t('Reveal password')}
                </Button>
                <Input
  ...
  return (
    <>
      ...
      <UpstreamTopupRatioConfirmDialog
        open={pendingUpstreamTopupRatio !== null}
        currentRatio={pendingUpstreamTopupRatio?.current ?? 1}
        upstreamRatio={pendingUpstreamTopupRatio?.upstream ?? 1}
        onKeepCurrent={() => setPendingUpstreamTopupRatio(null)}
        onUseUpstream={() => {
          if (!pendingUpstreamTopupRatio) return
          form.setValue(
