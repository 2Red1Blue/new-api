...
const newKeys = {
  ...
  fr: {
    'Cost multiplier for this upstream group.':
      'Multiplicateur de coût pour ce groupe upstream.',
    'Current password': 'Mot de passe actuel',
    'Fetch ratios from upstream and match this group to update the ratio field.':
      'Récupérez les ratios depuis l’upstream et faites correspondre ce groupe pour mettre à jour le champ du ratio.',
    'Fetch upstream ratios': 'Récupérer les ratios upstream',
    'Failed to fetch upstream password':
      'Échec de la récupération du mot de passe upstream',
    'How many USD-equivalent upstream credits 1 CNY buys. Empty or 0 means 1.':
      'Combien de crédits upstream équivalents en USD 1 CNY permet d’acheter. Vide ou 0 signifie 1.',
    'Leave empty to keep existing password':
      'Laissez vide pour conserver le mot de passe existant',
    'Optional. Keep blank if this upstream uses token or session auth.':
      'Facultatif. Laissez vide si cet upstream utilise une authentification par jeton ou session.',
    'Paste fetched upstream ratios JSON here':
      'Collez ici le JSON des ratios upstream récupérés',
    'Raw upstream ratios JSON snapshot used for matching and sync.':
      'Instantané JSON brut des ratios upstream utilisé pour la correspondance et la synchronisation.',
    'Reveal password': 'Afficher le mot de passe',
    'Save upstream login information and pricing metadata for ratio sync and alerts.':
      'Enregistrez les informations de connexion upstream et les métadonnées tarifaires pour la synchronisation des ratios et les alertes.',
    'Shown in insufficient balance notifications.':
      'Affiché dans les notifications de solde insuffisant.',
    ...
    'Used when logging in to the upstream site before fetching ratios.':
      'Utilisé lors de la connexion au site upstream avant de récupérer les ratios.',
    'Verification required to reveal the saved upstream password.':
      'Une vérification est requise pour afficher le mot de passe upstream enregistré.',
  },
  ja: {
    'Cost multiplier for this upstream group.':
      'このアップストリームグループのコスト倍率です。',
    'Current password': '現在のパスワード',
    'Fetch ratios from upstream and match this group to update the ratio field.':
      'アップストリームから倍率を取得し、このグループに一致した値で倍率欄を更新します。',
    'Fetch upstream ratios': '上流倍率を取得',
    'Failed to fetch upstream password':
      'アップストリームのパスワード取得に失敗しました',
    'How many USD-equivalent upstream credits 1 CNY buys. Empty or 0 means 1.':
      '1 CNY で購入できる USD 相当のアップストリームクレジット量です。空欄または 0 は 1 を意味します。',
    'Leave empty to keep existing password':
      '既存のパスワードを保持するには空欄のままにしてください',
    'Optional. Keep blank if this upstream uses token or session auth.':
      '任意です。このアップストリームがトークンまたはセッション認証を使う場合は空欄のままにしてください。',
    'Paste fetched upstream ratios JSON here':
      '取得したアップストリーム倍率 JSON をここに貼り付けます',
    'Raw upstream ratios JSON snapshot used for matching and sync.':
      '照合と同期に使うアップストリーム倍率の生 JSON スナップショットです。',
    'Reveal password': 'パスワードを表示',
    'Save upstream login information and pricing metadata for ratio sync and alerts.':
      '倍率同期とアラートのために、アップストリームのログイン情報と価格メタデータを保存します。',
    'Shown in insufficient balance notifications.':
      '残高不足通知に表示されます。',
    ...
    'Used when logging in to the upstream site before fetching ratios.':
      '倍率取得前にアップストリームサイトへログインするときに使います。',
    'Verification required to reveal the saved upstream password.':
      '保存済みのアップストリームパスワードを表示するには認証が必要です。',
  },
  ru: {
    'Cost multiplier for this upstream group.':
      'Коэффициент стоимости для этой upstream-группы.',
    'Current password': 'Текущий пароль',
    'Fetch ratios from upstream and match this group to update the ratio field.':
      'Получите коэффициенты из upstream и сопоставьте эту группу, чтобы обновить поле коэффициента.',
    'Fetch upstream ratios': 'Получить upstream-коэффициенты',
    'Failed to fetch upstream password':
      'Не удалось получить upstream-пароль',
    'How many USD-equivalent upstream credits 1 CNY buys. Empty or 0 means 1.':
      'Сколько upstream-кредитов в эквиваленте USD можно купить за 1 CNY. Пусто или 0 означает 1.',
    'Leave empty to keep existing password':
      'Оставьте пустым, чтобы сохранить текущий пароль',
    'Optional. Keep blank if this upstream uses token or session auth.':
      'Необязательно. Оставьте пустым, если этот upstream использует аутентификацию по токену или сессии.',
    'Paste fetched upstream ratios JSON here':
      'Вставьте сюда JSON с полученными upstream-коэффициентами',
    'Raw upstream ratios JSON snapshot used for matching and sync.':
      'Необработанный JSON-снимок upstream-коэффициентов для сопоставления и синхронизации.',
    'Reveal password': 'Показать пароль',
    'Save upstream login information and pricing metadata for ratio sync and alerts.':
      'Сохраните данные входа upstream и ценовые метаданные для синхронизации коэффициентов и оповещений.',
    'Shown in insufficient balance notifications.':
      'Показывается в уведомлениях о недостаточном балансе.',
    ...
    'Used when logging in to the upstream site before fetching ratios.':
      'Используется при входе на upstream-сайт перед получением коэффициентов.',
    'Verification required to reveal the saved upstream password.':
      'Чтобы показать сохранённый upstream-пароль, требуется подтверждение.',
  },
  vi: {
    'Cost multiplier for this upstream group.':
      'Hệ số chi phí cho nhóm upstream này.',
    'Current password': 'Mật khẩu hiện tại',
    'Fetch ratios from upstream and match this group to update the ratio field.':
      'Lấy tỷ lệ từ upstream và khớp nhóm này để cập nhật trường tỷ lệ.',
    'Fetch upstream ratios': 'Lấy tỷ lệ upstream',
    'Failed to fetch upstream password':
      'Không thể lấy mật khẩu upstream',
    'How many USD-equivalent upstream credits 1 CNY buys. Empty or 0 means 1.':
      '1 CNY mua được bao nhiêu tín dụng upstream quy đổi theo USD. Để trống hoặc 0 nghĩa là 1.',
    'Leave empty to keep existing password':
      'Để trống để giữ nguyên mật khẩu hiện có',
    'Optional. Keep blank if this upstream uses token or session auth.':
      'Không bắt buộc. Để trống nếu upstream này dùng xác thực bằng token hoặc phiên.',
    'Paste fetched upstream ratios JSON here':
      'Dán JSON tỷ lệ upstream đã lấy vào đây',
    'Raw upstream ratios JSON snapshot used for matching and sync.':
      'Ảnh chụp JSON tỷ lệ upstream thô dùng để đối chiếu và đồng bộ.',
    'Reveal password': 'Hiện mật khẩu',
    'Save upstream login information and pricing metadata for ratio sync and alerts.':
      'Lưu thông tin đăng nhập upstream và siêu dữ liệu giá để đồng bộ tỷ lệ và cảnh báo.',
    'Shown in insufficient balance notifications.':
      'Hiển thị trong thông báo số dư không đủ.',
    ...
    'Used when logging in to the upstream site before fetching ratios.':
      'Dùng khi đăng nhập vào trang upstream trước khi lấy tỷ lệ.',
    'Verification required to reveal the saved upstream password.':
      'Cần xác minh để hiển thị mật khẩu upstream đã lưu.',
  },
}
