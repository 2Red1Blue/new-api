...
export function ChannelMutateDrawer({
  ...
  const [isSavingKeyOnly, setIsSavingKeyOnly] = useState(false)
    ...
    try {
      ...
      if (currentGroup) {
        try {
          ...
          const currentGroupSnapshot = Object.entries(parsedRaw).find(
            ([group]) => group.trim() === currentGroup
          )?.[1]
          if (
            currentGroupSnapshot &&
            typeof currentGroupSnapshot === 'object' &&
            !Array.isArray(currentGroupSnapshot)
          ) {
            const rpmLimit = Number(
              (currentGroupSnapshot as Record<string, unknown>).rpm_limit
            )
            if (Number.isFinite(rpmLimit) && rpmLimit >= 0) {
              form.setValue('upstream_rpm_limit', Math.floor(rpmLimit), {
                shouldDirty: true,
                shouldValidate: true,
              })
            }
          ...
          // Keep legacy ratio-only snapshots working without surfacing parse noise.
        }
      }
  ...
  const handleSaveKeyOnly = useCallback(async () => {
    if (!isEditing || !currentRow || !channelData?.data) return

    const keyValue = form.getValues('key')?.trim() || ''
    if (!keyValue) {
      form.setError('key', {
        type: 'manual',
        message: t('Please enter API key first'),
      })
      return
    }

    const baseDefaults = transformChannelToFormDefaults(channelData.data)
    const keyOnlyFormData: ChannelFormValues = {
      ...baseDefaults,
      key: keyValue,
      key_mode: isMultiKeyChannel ? form.getValues('key_mode') : 'append',
    }
    ...
    if (isMultiKeyChannel && keyOnlyFormData.key_mode) {
      payload.key_mode = keyOnlyFormData.key_mode
    }

    setIsSavingKeyOnly(true)
    try {
      ...
      if (response.success) {
        ...
        setChannelKey(null)
        const nextDefaults = transformChannelToFormDefaults(
          response.data ?? channelData.data
        )
        form.reset({
          ...nextDefaults,
          key: '',
      ...
      toast.error(getErrorMessage(error) || t('Failed to save API key'))
    } finally {
      setIsSavingKeyOnly(false)
    }
  }, [
    isEditing,
    currentRow,
          ...
          <Form {...form}>
            ...
            >
                          ...
                          <div className='flex flex-col gap-2'>
                            ...
                            {isEditing && (
                              <Button
                                ...
                                disabled={isSubmitting || isSavingKeyOnly}
                                className='w-fit'
                              >
                                {isSavingKeyOnly && (
              ...
              <div className='bg-card space-y-4 rounded-xl border p-5'>
                ...
                />
                {isEditing && channelData?.data?.upstream_profile && (
                  <div className='text-muted-foreground space-y-2 border-t pt-3 text-xs'>
                    <div>
                      <span className='text-foreground font-medium'>
                        {t('Last auto-priority scan time')}:
                      </span>{' '}
                      {formatUnixTime(
                        channelData.data.upstream_profile.auto_priority_updated_at
                      )}
                    </div>
                    <div>
                      <span className='text-foreground font-medium'>
                        {t('Last auto-priority scan result')}:
                      </span>{' '}
                      <span className='break-all'>
                        {channelData.data.upstream_profile.auto_priority_reason ||
                          '-'}
                      </span>
                    </div>
                  </div>
