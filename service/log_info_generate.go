...
func AppendRelayLogAdminInfo(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, other *model.LogOther) {
	...
	if anyInfo, ok := ctx.Get(ginKeyChannelAffinityLogInfo); ok && anyInfo != nil {
		other.SetAdmin("channel_affinity", anyInfo)
	}
	if anyInfo, ok := ctx.Get("channel_selection_info"); ok && anyInfo != nil {
		other.SetAdmin("channel_selection", anyInfo)
	}
}
...
func GenerateTextOtherInfo(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelRatio, groupRatio, completionRatio float64,
	...
	if timing := relayInfo.TimingSinceStartMs(); len(timing) > 0 {
		other.SetPublic("relay_timing_ms", timing)
	}
	if timingMeta := relayInfo.TimingMetaSnapshot(); len(timingMeta) > 0 {
		other.SetPublic("relay_timing_meta", timingMeta)
	}
	if relayInfo.IsModelMapped {
		other.SetPublic("is_model_mapped", true)
		other.SetPublic("upstream_model_name", relayInfo.UpstreamModelName)
	}
	if relayInfo.PricingModelName != "" && relayInfo.PricingModelName != relayInfo.OriginModelName {
		other.SetPublic("pricing_model_name", relayInfo.PricingModelName)
	}
...
func appendStreamStatus(relayInfo *relaycommon.RelayInfo, other any) {
	...
	switch target := other.(type) {
	case *model.LogOther:
		target.SetPublic("stream_status", streamInfo)
	case map[string]any:
		target["stream_status"] = streamInfo
	}
}
