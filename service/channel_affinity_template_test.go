...
func TestGetPreferredChannelByAffinity_RequestHeaderKeySource(t *testing.T) {
	...
	require.NoError(t, cache.SetWithTTL(cacheKeySuffix, ChannelAffinityBinding{ChannelID: 9528, RealGroup: "default"}, time.Minute))
	t.Cleanup(func() {
...
func TestClearCurrentChannelAffinityCache(t *testing.T) {
	...
	cache := getChannelAffinityCache()
	require.NoError(t, cache.SetWithTTL(cacheKeySuffix, ChannelAffinityBinding{ChannelID: 9527, RealGroup: "default"}, time.Minute))
	t.Cleanup(func() {
		_, _ = cache.DeleteMany([]string{cacheKeySuffix})
	})
		...
		for _, tc := range []struct {
			...
			t.Run(globalMode+"/"+tc.name, func(t *testing.T) {
				...
				*setting = snapshot.Affinity
				...
				cache := getChannelAffinityCache()
				require.NoError(t, cache.SetWithTTL(cacheKey, ChannelAffinityBinding{ChannelID: 1, RealGroup: "default"}, time.Minute))
				...
				if expected == "off" {
					...
					require.NoError(t, err)
					assert.True(t, found)
					assert.Equal(t, ChannelAffinityBinding{ChannelID: 1, RealGroup: "default"}, cached, "off does not refresh session bindings")
